// =====================================================================
// GR EXPRESSO - Controller: Telemetria (integração real com o Launcher ETS2)
// =====================================================================
...
const TelemetriaController = {
  ...
  iniciarViagem: asyncHandler(async (req, res) => {
    ...
    const viagem = await ViagemModel.criar({
      ...
      motorista_nome: motorista?.nome || 'GR EXPRESSO',
      caminhao_id: caminhao.id,
      origem, destino,
      ...
      observacoes: 'Viagem criada automaticamente pelo Launcher GR EXPRESSO (telemetria do jogo).'
    });
  ...
  emitirNotaFiscal: asyncHandler(async (req, res) => {
    ...
    const nota = await NotaFiscalModel.criar({
      viagem_id: id,
      cliente: viagem.motorista_nome || 'GR EXPRESSO',
      descricao_carga: viagem.carga || 'Carga geral',
      valor_total: viagem.valor_frete || 0,
      data_emissao: new Date(),
      status: 'emitida',
      observacoes: 'Nota fiscal gerada automaticamente pelo Launcher GR EXPRESSO ao iniciar a viagem.'
    });
