// =====================================================================
// GR EXPRESSO - Página: Mapa das Entregas (Leaflet)
// =====================================================================

const CORES_STATUS_MAPA = {
  agendada: '#FFB020', em_andamento: '#3AA6FF', concluida: '#A4FF00', cancelada: '#FF4D4F'
};
...
(async function () {
  ...
  try {
    ...
    viagens.forEach((v) => {
      ...
      L.circleMarker(origem, { radius: 7, color: '#0B3D2E', fillColor: cor, fillOpacity: 0.9, weight: 2 })
        .addTo(mapa)
        .bindPopup(`<b>${v.codigo}</b><br>Origem: ${v.origem}<br>Motorista: ${v.motorista_nome}`);

      L.circleMarker(destino, { radius: 7, color: '#0B3D2E', fillColor: cor, fillOpacity: 0.9, weight: 2 })
        .addTo(mapa)
