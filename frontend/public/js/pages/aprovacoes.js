// =====================================================================
// FELIPINHO LAUNCHER - Página: Aprovações de Acesso (RH, Diretoria, Admin)
// =====================================================================
...
async function decidirUsuario(id, novoStatus, nome) {
  const confirmado = await Swal.fire({
    ...
    confirmButtonColor: novoStatus === 'aprovado' ? '#0B0B0B' : '#FF4D4F',
...
const CARGOS = {
  admin:     { label: 'Administrador', cor: 'var(--verde-limao)', bg: 'rgba(212, 160, 23,0.12)' },
...
async function alterarStatus(id, status, nome) {
  ...
  const { isConfirmed } = await Swal.fire({
    ...
    cancelButtonText: 'Cancelar',
    confirmButtonColor: status === 'aprovado' ? '#0B0B0B' : '#dc3545'
  });
